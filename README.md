...
1. Add the marketplace:
   ...
   ```
   /plugin marketplace add <user>/claude-skills-loader
   ```
...
If two or more `SKILL.md` files live in directories with the **same name** (e.g., `packages/foo/utils/SKILL.md` and `apps/bar/utils/SKILL.md`), the plugin detects the collision and prefixes the symlink with the parent directory name:

```
skills/
  foo--utils -> /absolute/path/to/packages/foo/utils
  bar--utils -> /absolute/path/to/apps/bar/utils
```
